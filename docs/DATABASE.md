...
| 077 | **Group chat fixes (v1.30.3):** `conversations.role_b` dropped NOT NULL (075 missed it - every `create_chat_group` insert failed); `group_kind` column (`school`/`friends` per the Penpot type selector) + `p_kind` parameter; group name relaxed to 1-80 chars, all characters allowed |
| 078 | **Group editing (v1.31):** `conversations.cover_url` + public `chat-covers` bucket (`{school_id}/{conversation_id}/{uuid}.ext`, school-read + member-write policies) + owner-only `update_chat_group` RPC for rename/cover |
