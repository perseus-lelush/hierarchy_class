...
test("APK release metadata matches the distributed artifact contract", () => {
  ...
  assert.equal(APK_RELEASE.versionCode, 131003);
  assert.equal(APK_RELEASE.sizeBytes, 7729845);
});

test("download URL is versioned and served from the public downloads path", () => {
  assert.equal(apkDownloadUrl(), `/downloads/hierarchy-class-v1.31.3.apk`);
